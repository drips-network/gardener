"""
Fixture-based Python micro repo test
"""
...
def test_python_fixture_graph_matches_spec(offline_mode):
    """
    Verify end-to-end dependency graph generation for the Python fixture

    Checks for expected nodes (files, packages, components) and edges
    (imports_local, imports_package, uses_component, contains_component)
    based on the known structure of the tests/fixtures/python directory
    """
    # Convert relative path to absolute to avoid path duplication in SecureFileOps
    fixture_repo_path = os.path.abspath("tests/fixtures/python")
    assert os.path.isdir(fixture_repo_path), f"Fixture directory not found: {fixture_repo_path}"

    # Ensure URL resolution is offline/deterministic
    with offline_mode.set_responses({}):
        results = run_analysis(
            repo_path=fixture_repo_path,
            output_prefix="test_python_fixture_output",
            verbose=False,
            minimal_outputs=True,
            focus_languages_str="python",
    ...
    spec = load_graph_spec("tests/data/specs/python_micro.yml")
